...
/*my.ini should have this entry: secure_file_priv = C:/data/ */

/* MySql Workbench restore workplace error: Could not read contents of */
/* To fix, delete C:\Users\pm250\AppData\Roaming\MySQL\Workbench\sql_workspaces\*.* */

/* Run  */

use covid19_2;
/* Add stored procedures (see below):
Covid19_CreateTables1() and Covid19_CreateTables2()
*/
...
/* We just need to add a record to this table.  The processing routines will determine the date based on what is in the history table, and update it accordingly */
...
Insert into covid_population_county(fips, county_cd, state_cd, state, county, population)
values('60', '000', 'AS', 'American Samoa', 'American Samoa', '1802701'),
('66', '000', 'GU', 'Guam', 'Guam', '1802705'),
('69', '000', 'MP', 'Northern Mariana Islands', 'Northern Mariana Islands', '1779809'),
('72', '000', 'PR', 'Puerto Rico', 'Puerto Rico', '1779808'),
('74', '000', 'UM', 'U.S. Minor Outlying Islands', 'U.S. Minor Outlying Islands', '1878752'),
('78', '000', 'VI', 'U.S. Virgin Islands', 'U.S. Virgin Islands', '1802710');
...
drop procedure Covid_LoadCycle;
...
BEGIN
    ...
    /* I have 3 sections so I can put the US totals on top (section 1), order the rest of the states by infection rate (section 2), and follow them with the US territories (Guam, US Virgin Islands, etc = section 3)  */
    update coviddata_state_chg chg
    set section = 3 
    where chg.fips in (60, 66, 69, 72, 74, 78);
	...
	/* If this cycle already exists, then delete it */	
...
/* 00:19:17	CALL Covid_LoadNewCycles()	Error Code: 1366. Incorrect decimal value: '1,112' for column 'casesPer100k' at row 1	33.844 sec */
     ...
     Update coviddata_state_chg chg 
        ...
        set description = concat(chg.period, ' high is ', FORMAT(max1.cases_chg, 0), ' cases, occurring on ', DATE_FORMAT( max1.last_date, "%b %e"), ',', DATE_FORMAT(max1.last_date, "%Y"), '. ');

END$$
DELIMITER ;


drop procedure Covid_LoadNewCycles;
...
/* Load tables into local WordPress database for display in WordPress app */
truncate table local.wp_covidrpt_state_history;
insert into local.wp_covidrpt_state_history(date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, rate_chg, section, max_value)
select date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, rate_chg, section, max_value
from coviddata_state_history order by date_end desc, period_days, casesPer100k desc;


truncate table local.wp_covidrpt_state_chg;
insert into local.wp_covidrpt_state_chg(date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, pop, casesPer100k, rate_chg, section, max_value, description)
select date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, pop, casesPer100k, rate_chg, section, max_value, description
from coviddata_state_chg order by date_end desc, period_days, casesPer100k desc;
...
/* Create export files to load into remote database */

drop procedure Covid_CreateExportFile;
...
/*   No longer saving covidrpt_history - It's too big!
...
use covid19_2;
/* delete from coviddata_state_history where date_end='2022-06-09'; */
...
/* load data to remote site */

/*
truncate table wp_covidrpt_state_chg;

LOAD DATA INFILE 
    'C:/data/covidrpt_chg_2022-06-13.csv'
    INTO TABLE aiw_covidrpt_state_chg
    FIELDS TERMINATED BY ';' 
    LINES TERMINATED BY '\n'
    IGNORE 0 LINES;
    
      Insert into aiw_covidrpt_state_history(date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, section)
	  Select date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, section 
	  from aiw_covidrpt_state_chg;
    
Or call stored procedure below    

call Covid_Load_covidrpt_state_chg;

*/


/* Routine for loading new days totals to a remote website */
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Covid_Load_covidrpt_state_chg`()
BEGIN

     set @date_end = (select max(date_end) from wp_covidrpt_state_chg);


	/* If this cycle already exists, then delete it */	
	 delete from wp_covidrpt_state_history where date_end=@date_end;
     
	 Insert into aiw_covidrpt_state_history(date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, section)
	 Select date_end, date_begin, period, period_days, state, fips, state_cd, cases_begin, cases_end, cases_chg, cases_avg, deaths_begin, deaths_end, deaths_chg, deaths_avg, casesPer100k, section 
     from aiw_covidrpt_state_chg where date_end=@date_end;
...
/* When loading Covid19 data from the beginning from within MySql Workbench, this routine will report its progress by creating a new file for every day processed*/
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Covid_ReLoadNewCyclesFromBeginning`(inLogThis TINYINT)
BEGIN

set @numRecs = (SELECT count(*) FROM coviddata_state_chg);
set @inLogThis = inLogThis;
...
WHILE DATEDIFF(@date_end1, @date_begin1) > 0  DO
        ...
        IF @inLogThis = 1 THEN
            SET @strdate=DATE_FORMAT(@date_begin1,'%Y-%m-%d');
			SET @filename = concat("C:/data/last_date_Processed_", @strdate, ".csv");
			SET @CMD = CONCAT("SELECT Process_ID, date_begin, date_end, period, period_days FROM covid19_3.covid_process_date INTO OUTFILE '", @filename, 
			"' FIELDS OPTIONALLY ENCLOSED BY '\"' TERMINATED BY ';'", " LINES TERMINATED BY '\r\n';");
			PREPARE statement FROM @CMD;
			EXECUTE statement;
		END IF;
